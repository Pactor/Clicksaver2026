...
public sealed class HookViewModel : ObservableObject, IAsyncDisposable
{
    ...
    /// <summary>Rolls a mission with a prebuilt payload (empty, 1-byte tick, or tick + 6 sliders).</summary>
    public Task<bool> SendRollAsync(int processId, uint id, ReadOnlyMemory<byte> payload, CancellationToken cancellation) =>
        this.commandServer.SendAsync(processId, HookProtocol.RequestMissionsCommand, id, payload, cancellation);
