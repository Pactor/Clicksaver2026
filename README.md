...
**Status: early.** The hook, the pipe to the app and message capture work. The mission parser is
being rebuilt from captures of the current client, so the Missions tab is still empty.
...
| `src/ClickSaver2026.Core` | C# .NET 10. Pipe server, attaching to clients, capture files, and (to come) the mission parser, resource database and watch lists. |
